...
type reviewTestcase struct {
	name        string
	workerCount int
	callGroups  []reviewCallGroup
}

type reviewCallGroup struct {
	calls []reviewCall
}
...
func TestReview(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		testCase.callGroups[0].calls = append(
			testCase.callGroups[0].calls,
			reviewCall{
				assets:             defaultReviewTestAssets,
				scaleFactor:        16,
				wantViolationCount: 1,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for callGroupIdx, callGroup := range tc.callGroups {
				var groupDone sync.WaitGroup
				for callIdx, call := range callGroup.calls {
					groupDone.Add(1)
					go func(cgIdx, cIdx int, call reviewCall) {
						...
						if err != nil {
							t.Fatalf("review error in callgroup %d call %d: %s", cgIdx, cIdx, err)
						}
